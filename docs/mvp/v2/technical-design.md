...
谱例固定为一条标准调弦吉他轨道和 8 个连续的 `4/4` 小节。所有小节的 rhythm/tick 总和均为 `3840`，同时覆盖四分、八分、十六分、三十二分、单附点与双附点，确保数据可直接用于命中、单音命令和时值图形回归。8 小节至少覆盖：

| 小节 | 数据重点                      | v2 覆盖目的                              |
| ---- | ----------------------------- | ---------------------------------------- |
| 1    | MVP1 风格混合节奏、Am 标记    | 附点、三十二分、开放弦、和弦标记与连梁。 |
| 2    | 八分分解和弦                  | 同 beat 多弦保留与单弦覆盖。             |
| 3    | 十六分走句、高品位 `12`、`24` | 两位品位草稿、最大品位和多层连梁。       |
| 4    | 双附点与附点八分              | 附点锚点及短时值混合。                   |
| 5    | 半音符低音与双音              | 不同节奏列宽、双小节线和多弦音符。       |
| 6    | 高音弦旋律                    | 第一、二弦命中边界。                     |
| 7    | 低音与高音交替                | 第二行中部的丰富可编辑目标。             |
| 8    | 终止小节线 `final`            | 最后一行、最后小节和收束和弦。           |

该 fixture 不包含休止、连音组、技巧、歌词或和弦图；附点与和弦标记仅作为现有 layout 回归数据，不代表 v2 开放对应编辑入口。小节数量和不同节奏列宽足以在测试中通过较小的 `systemWidth` 断为多行，也可以通过较大的 `systemWidth` 验证单行行为。
