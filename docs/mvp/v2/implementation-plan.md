...
- 新增 `packages/lxm-editor/example/example-mvp2.json.ts`：
  ...
  - 固定一条标准调弦吉他轨道、8 个连续 `4/4` 小节、每小节四个四分音符 beat。
  - 使用稳定且全局唯一的 `track`、`measure`、`beat`、`note` ID。
  - 覆盖开放弦、同拍多弦、高品位 `12/24`、稀疏音符、全部六根弦与最终小节线。
  - 不含 v2 非范围内的休止、附点、连音、技巧、歌词或和弦图。
...
- 所有 ID 唯一，所有品位在 `0–24` 范围，所有 beat tick 与四分音符时值匹配。
