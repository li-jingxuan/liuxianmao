...
目标 Beat 必须包含至少两颗 Note，技巧的纵向跨度由这些 Note 所在弦的最终 Y 坐标推导。领域模型本身能够表达“哪些音参与演奏”以及“以什么方式演奏”，但当前页面把两层记谱同时画出：
...
最初方案曾考虑在添加扫弦/琶音时删除 Note，并用 `fromString/toString` 代替具体音符。复审后该方向被否决，原因是 Note 不只是当前屏幕上的品位文本，它还是：
...
- 扫弦/琶音的跨度继续从目标 Beat 的 Note 弦位置推导；
- 目标 Beat 的 Note 继续参与时值、技巧几何、语义校验和后续编辑；
- 最终 SVG 不展示该 Beat 的任何品位数字；
- 删除扫弦/琶音后，原 Note 在下一次布局中自然重新显示；
- 修改 Note 时保留仍然合法的扫弦/琶音，并自动更新技巧跨度；
- 删除 Note 导致少于两颗时，继续按既有领域规则级联删除扫弦/琶音；
- 同一 Beat 的扫弦与琶音继续互斥；
...
- 删除或迁移现有 Note 数据；
- 为扫弦/琶音新增 `fromString/toString`；
- 表达“和弦包含 6～1 弦，但只扫 6～3 弦”的局部范围；
...
“扫弦/琶音只作用于和弦中的部分弦”若成为明确需求，应另行增加显式弦范围字段；不能在本 fix 中用选区端点暗中改变持久化语义。
...
视觉跨度继续由目标 Beat 的 Note 外侧弦位置决定。以 Note 位于 6～3 弦为例：
...
同一 Beat 存在 strum 或 arpeggio
  → 显示扫弦/琶音记号
  → 隐藏该 Beat 的全部 fret Note 文本
  → 保留 rhythm stem、beam、flag、dot 和 sustain marks
```

不使用“半透明品位”“先画文字再用白色背景盖住”或“技巧图层放在文字上方”等视觉伪互斥。最终 SVG 中不应创建这些品位 `<text>` 元素。

本 fix 暂不改变其他 Note 技巧的展示。若实际验收出现无品位背景下的 bend、泛音、H/P 等标记语义不清，应单独定义完整的 technique precedence matrix；不能在没有产品规则时静默删除领域技巧或扩大隐藏范围。
...
- 同一 Beat 的所有 Note 一起隐藏，不按技巧路径覆盖到的弦范围做部分过滤；
- 输出顺序保持不变；
- 不原地修改数组。
...
- `LXMTechniqueSchema`；
- `ILXMTechniqueDraft`；
- `strum.stroke` 与 `arpeggio.direction`；
- loader 的 strict schema；
- 扫弦/琶音至少两颗 Note 的规则；
- 同 Beat 扫弦与琶音互斥规则。
...
### 7.3 Note 编辑沿用既有引用生命周期

当扫弦/琶音存在时：

- 修改某颗 Note 的 fret：技巧保留，跨度不变；
- 在新弦增加 Note：技巧保留，若新弦扩展外侧范围，下一次布局自动扩大记号；
- 删除内部 Note 后仍至少两颗：技巧保留，跨度按剩余 Note 更新；
- 删除外侧 Note 后仍至少两颗：技巧保留，跨度收缩；
- 删除到少于两颗：`pruneInvalidTechniques` 继续级联删除技巧，剩余 Note 重新显示；
...
不要求用户拖选起止弦。显示范围由 Beat Note 自动决定。
...
packages/lxm-editor/src/layout/
  technique-layout.ts          # 最终应用 fret visibility projection
...
- core types/schema/semantic validation；
...
it.each(["strum", "arpeggio"])(
  ...
  (type) => {
    // fixture 的目标 Beat 保留至少两颗真实 Note
    // 添加对应 technique
    // buildLayout
...
- strum Beat 的全部 Note 从最终 `measure.notes` 移除；
- arpeggio Beat 的全部 Note从最终 `measure.notes` 移除；
...
- 增加外侧 Note 后技巧跨度扩大，所有该 Beat Note 仍隐藏；
- 删除外侧 Note但仍至少两颗时跨度收缩；
- 删除到一颗时技巧级联删除，剩余 Note 重新显示；
...
1. 在一个包含 6～3 弦具体品位的 Beat 添加 down strum；确认只显示 `6 弦 → 3 弦` 方向的直线箭头，不显示品位。
...
6. 在更外侧弦增加 Note；确认技巧跨度自动扩大。删除该 Note 后范围自动收缩。
7. 删除到只剩一颗 Note；确认技巧按既有规则级联删除，剩余品位显示。
...
- 最终 `ILXMLayout` 不包含被扫弦/琶音覆盖 Beat 的 fret Note layout；
