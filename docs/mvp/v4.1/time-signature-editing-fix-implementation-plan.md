...
## 12. 实施完成记录（2026-08-12）

### 状态

- 拍号编辑 Fix 的核心命令、容量协调、拍组连梁、Store 历史与选区、页面工具均已实现；
- 当前等待用户代码审查与产品验收；
- 未修改 schema version，也没有收窄既有文档可加载的拍号结构。

### 已完成能力

- 页面可编辑 `2/4`、`3/4`、`4/4`、`6/8`；选项统一来自核心白名单；
- 支持“仅当前小节”和“从当前小节至下一拍号变化”两种范围；
- 多小节修改先完成全部规划再一次提交，中间任一小节失败时不产生部分文档；
- 全休止小节按新拍号单位拍重建，含真实内容小节只重建尾部连续容量休止；
- 缩容会截断真实内容或导致和弦标记越界时返回明确错误；
- `3/4` 按三个四分拍组、`6/8` 按两个附点四分拍组生成连梁；
- 非白名单拍号继续允许加载，连梁保守降级为整小节单组；
- 单小节或多小节修改只产生一条历史；undo/redo 后焦点保留在原目标小节；
- 尾部休止 ID 被替换时，选区回到目标小节首拍并保留原弦号。
- 页面为保持工具栏简洁，固定采用“从当前小节至下一拍号变化”范围；核心领域命令仍保留
  `measure` scope，供自动化测试和未来高级编辑入口复用。

### 自动化检查

- 核心包拍号相关聚焦测试：52 个通过；
- website Store 与交互聚焦测试：13 个通过；
- `pnpm type-check`：通过；
- `pnpm lint`：通过；
- `pnpm build`：通过。Turbopack 构建 CSS 时需要创建本地子进程并绑定端口，因此在允许该本地能力后完成；
- `pnpm test`：核心包共 139 个测试，135 个通过。剩余 4 个失败均来自实现开始前已存在的
  `compact.measurePaddingX` 从 `8` 调整为 `12`、但间距/断行/命中测试仍保留旧 `8px` 与 `4+4`
  基线；本 Fix 未覆盖该工作区改动，也未把它混入拍号提交范围。

### 浏览器验收

- 环境：Codex 内置 Chromium 浏览器，桌面视口约 `1280 × 720`，固定 A4 页面；
- 在第 4 小节后新增空白小节，将第 5 小节以“仅当前小节”修改为 `3/4`；
- 验证第 5 小节显示 `3/4`，下一小节自动恢复并显示 `4/4`；
- 验证真实内容无法安全缩容时显示明确错误，文档与历史不发生部分修改；
- 验证 undo 后工具恢复 `4/4`，redo 后恢复 `3/4`，焦点始终位于第 5 小节；
- 页面及控制台无 warning/error。

### 已知限制

- 自定义拍号、`5/8`/`7/8` 拍组配置、弱起小节和不完整小节仍不在 v4.1 范围；
- 不自动压缩、拆分、删除或跨小节移动真实音符；
- 不自动移动越界和弦标记；
- 不支持一次修改不连续小节、多轨道同步拍号和播放层变拍。
