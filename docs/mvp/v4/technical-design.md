...
本版本不实现 Beat 剪贴板。复制需求继续由已有的 `measure.copy` 覆盖完整小节；部分小节和乐句复制留到有明确使用证据后的独立候选版本。
...
| Beat/乐句复制粘贴        | 中等     | 半小节 riff 与短乐句复用     | 高：容量、tick、ID、未来技巧关系 | 延后至独立候选 |
...
Beat/乐句剪贴板只在实际试用确认部分小节重复录入是主要瓶颈后启动。其范围应单独定义完整 Beat payload、覆盖式或插入式粘贴、容量规则、跨小节策略、ID 重建以及未来技巧关系，不在 v4 选区 Module 中预埋剪贴板分支。MVP v4.1 已用于基础谱面标识与小节边界补丁。
