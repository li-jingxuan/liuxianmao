...
- 对 rest beat 执行合法 `note.set` 时，在同一次命令内转为 notes 并写入音符；非法输入保持原 rest 不变。`note.remove` 可维持 no-op 成功语义。
...
- 对 rest 写合法品位一次成功并自动转为 notes；非法品位保持原 document 与 revision 不变。
...
- 验证 overflow、对 rest 输入非法品位、删除最后小节、无 cursor 操作均显示可读错误且文档不变；合法品位可直接写入 rest。
...
| 2026-07-31 | 已完成 | 86 个核心测试、类型检查、lint、生产构建全部通过；完成 1440×1100 桌面浏览器交互与 A4 两行四小节验收，详见版本验收记录。 |
