...
> 状态：P1–P2 代码已实施，透明背景方案已撤回并恢复默认纯白背景，待效果集验证
> 算法版本：`bead-grid-constrained-v1`
> 方案日期：2026-08-14
> 影响范围：`apps/api` 的背景处理、网格采样、颜色量化、配置、响应元数据与测试，以及 `apps/web` 的背景选项、预览和豆数统计

> 2026-08-14 产品决策更新：实测 Seedream 不能稳定返回真实 Alpha，透明模式会产生
> `AI_TRANSPARENT_BACKGROUND_INVALID`。因此撤回公开 `transparent` 模式，恢复
> `simplify | solid | keep`；`solid` 默认并优先使用纯白 `#FFFFFF`，仍允许用户选择其他
> `#RRGGBB` 纯色。本文后续关于透明背景的章节保留为历史设计记录，不再是当前实现契约。
...
1. 为每个待移除颜色缓存“排除该颜色后的最近距离”。
2. 固定按已选色号和全部未选候选色号排序遍历。
3. 尝试用未选色替换一个已选色。
4. 仅当相对误差改善超过 `swap_min_relative_gain` 时接受。
5. 最多接受 2 次交换或达到最大轮次后停止。

最多接受两次严格改善；缓存排除距离后，每个交换候选只需线性扫描观察集。
...
最大网格仍有 24,336 个单元，但距离计算前先将相近 RGB 自适应聚合为最多 512 个带面积权重的颜色观察：

```text
M = min(聚合后颜色数, 512)
C = 264 个 MARD 候选色
M × C <= 135,168 个 CIEDE2000 距离
```

聚合从完整 RGB 开始；若观察数超过 512，则逐步降低每通道有效位数，并以桶内 RGB 加权平均值作为观察色。每个网格位置保存观察索引，最终再展开为完整 `rows`。Median Cut 不再参与最终调色板选择。
...
- CIEDE2000 标量实现继续匹配公开参考色对；
- 源色观察聚合保持总权重等于占用格数量；
- 相同输入的观察顺序、候选选择和最终结果完全确定。
...
| 风险 | 处理方式 |
| --- | --- |
| 全量 CIEDE2000 计算过慢 | 将源色聚合为最多 512 个带权观察，缓存贪心与交换距离 |
| 源色聚合损失细微颜色 | 使用桶内加权平均，并通过渐变、肤色和身份色效果集验证 |
