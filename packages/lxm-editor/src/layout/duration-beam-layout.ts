  ...
  for (let level = 1; level <= maxBeamLevel; level += 1) {
    ...
    const flushLevelMarks = () => {
      ...
      if (levelMarks.length >= 2) {
        ...
        const markIndex = group.findIndex(item => item.beatId === mark.beatId)
...
): ILXMDurationMarkLayout[] => sourceBeats.map(beat => {
...
export const layoutDurationBeams = (
  ...
  const sourceBeats = arraySortByKey<ILXMBeat>(measure.beats, 'tick')
  const beatLayoutMap = new Map(beatLayouts.map(layout => [layout.id, layout]))
